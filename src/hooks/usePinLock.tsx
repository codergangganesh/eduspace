...
export function PinLockProvider({ children }: { children: ReactNode }) {
  ...
  // Sync state with storage for active user session
  ...
  const unlockWithPin = useCallback(
    async (pin: string) => {
      ...
      if (res.success) {
        ...
        lastActivityRef.current = Date.now();
        try {
          localStorage.setItem(`eduspace_user_last_act_${userId}`, String(Date.now()));
        } catch (_) {}
      } else {
  ...
  // Unlock with Biometrics (Fingerprint / Touch ID / Windows Hello / Face ID)
  ...
  const enableBiometrics = useCallback(async () => {
    ...
    const displayName = profile?.full_name || "EduSpace User";
        ...
        if (isBiometricsSupported) {
          try {
            await pinLockService.registerBiometrics(
              userId,
              user?.email || "",
              profile?.full_name || "EduSpace User"
            );
          } catch (_) {}
        }
        refreshStatus();
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      try {
        const stored = localStorage.getItem(`eduspace_user_last_act_${userId}`);
        if (stored) {
