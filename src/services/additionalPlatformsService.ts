...
function extractUsername(input: string): string {
  ...
  clean = clean.replace(/^(https?:\/\/)?(www\.)?(hackerrank\.com\/profile\/|hackerrank\.com\/|hackerearth\.com\/@?|hackerearth\.com\/)/i, "");
...
/**
 * Fetches HackerRank statistics using official HackerRank REST APIs & multi-tiered CORS microservices.
 */
export async function fetchHackerRankStats(usernameInput: string): Promise<{
  ...
  // Tier 1: Dedicated Competitive Programming JSON Microservices (Fast & High Availability with CORS support)
  // 1a. Fetch Profile Model from hackerrank-profile-api
  try {
    const res = await fetch(`https://hackerrank-profile-api.vercel.app/${encodeURIComponent(username)}`, {
      cache: "no-store",
      signal: AbortSignal.timeout(6000),
    });
    if (res.ok) {
      const json = await res.json();
      if (json && (json.model || json.username)) {
        profileData = json.model || json;
      }
    }
  } catch { }

  // 1b. Fetch Badges from hackerrank-stats.tashif.codes
  try {
    const res = await fetch(`https://hackerrank-stats.tashif.codes/${encodeURIComponent(username)}/badges`, {
      cache: "no-store",
      signal: AbortSignal.timeout(6000),
    });
    if (res.ok) {
      const json = await res.json();
      if (json && Array.isArray(json.badges) && json.badges.length > 0) {
        badgesData = json.badges.map((b: any) => {
          let stars = 1;
          if (b.id && typeof b.id === "string") {
            const parts = b.id.split(":");
            const lastPart = parseInt(parts[parts.length - 1], 10);
            if (!isNaN(lastPart) && lastPart > 0) stars = Math.min(6, lastPart);
          }
          if (typeof b.stars === "number") stars = b.stars;
          return {
            badge_name: b.displayName || b.name || b.badge_name || "Domain Badge",
            stars: stars,
  ...
  // 1c. Fetch Profile Contributions / Question Count from hackerrank-stats.tashif.codes
  try {
    const res = await fetch(`https://hackerrank-stats.tashif.codes/${encodeURIComponent(username)}/profile`, {
      cache: "no-store",
      signal: AbortSignal.timeout(6000),
    });
    if (res.ok) {
  ...
  // Tier 2: Official REST API & Multi-tiered Fallbacks if needed
  if (!profileData) {
    const profileUrls = [
      `https://www.hackerrank.com/rest/hackers/${encodeURIComponent(username)}/profile?_t=${timestamp}`,
      `https://api.allorigins.win/get?url=${encodeURIComponent(`https://www.hackerrank.com/rest/hackers/${encodeURIComponent(username)}/profile?_t=${timestamp}`)}`,
      `https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(`https://www.hackerrank.com/rest/hackers/${encodeURIComponent(username)}/profile?_t=${timestamp}`)}`,
    ];
  ...
  // Fetch Real-Time Scores Data if not yet populated
  if (!scoresData) {
    const scoreUrls = [
      `https://www.hackerrank.com/rest/hackers/${encodeURIComponent(username)}/scores_data?_t=${timestamp}`,
      `https://api.allorigins.win/get?url=${encodeURIComponent(`https://www.hackerrank.com/rest/hackers/${encodeURIComponent(username)}/scores_data?_t=${timestamp}`)}`,
      `https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(`https://www.hackerrank.com/rest/hackers/${encodeURIComponent(username)}/scores_data?_t=${timestamp}`)}`,
    ];

    for (const sUrl of scoreUrls) {
      try {
        const res = await fetch(sUrl, { cache: "no-store", signal: AbortSignal.timeout(5000) });
        if (res.ok) {
          let json: any = null;
          try {
            const text = await res.text();
  ...
  // Fetch Badges from REST API if empty
  if (badgesData.length === 0) {
    const badgeUrls = [
      `https://www.hackerrank.com/rest/hackers/${encodeURIComponent(username)}/badges?_t=${timestamp}`,
      `https://api.allorigins.win/get?url=${encodeURIComponent(`https://www.hackerrank.com/rest/hackers/${encodeURIComponent(username)}/badges?_t=${timestamp}`)}`,
      `https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(`https://www.hackerrank.com/rest/hackers/${encodeURIComponent(username)}/badges?_t=${timestamp}`)}`,
    ];

    for (const bUrl of badgeUrls) {
      try {
        const res = await fetch(bUrl, { cache: "no-store", signal: AbortSignal.timeout(5000) });
        if (res.ok) {
          let json: any = null;
          try {
            const text = await res.text();
  ...
  const certUrls = [
    ...
    `https://api.allorigins.win/get?url=${encodeURIComponent(`https://www.hackerrank.com/community/v1/hackers/${encodeURIComponent(username)}/certificates?_t=${timestamp}`)}`,
  ];

  for (const cUrl of certUrls) {
    try {
      const res = await fetch(cUrl, { cache: "no-store", signal: AbortSignal.timeout(5000) });
      if (res.ok) {
        let json: any = null;
        try {
          const text = await res.text();
  ...
  // Fallback to profileData certificates if available
  ...
  const rawBadges: HackerRankBadge[] = badgesData
    .filter((b: any) => (b.stars && b.stars > 0) || (b.solved && b.solved > 0) || b.badge_name || b.title || b.displayName)
    .map((b: any) => ({
      badge_name: b.badge_name || b.badge_type || b.displayName || b.title || b.name || "Domain Badge",
  ...
  // Calculate Total Solved from Badges, Question Count, or Solved challenges
  let totalSolved = 0;
  if (statsData?.contributions?.questionCount) {
    totalSolved = statsData.contributions.questionCount;
  }
  ...
  } else if (totalSolved === 0 && profileData?.level && profileData.level > 0) {
    totalSolved = profileData.level * 10;
  }
  ...
  const realScore = scoresData?.score || profileData?.score || statsData?.contributions?.points || undefined;

  if (profileData || scoresData || statsData || parsedBadges.length > 0 || parsedCerts.length > 0) {
    return {
      data: {
        username,
        name,
