...
export const passkeyService = {
  /**
   * List all enrolled Passkeys for the currently authenticated student or lecturer.
   */
   ...
   */
  async authenticateWithPasskey(captchaToken?: string): Promise<{ data: any; error: string | null }> {
    try {
      const auth = supabase.auth as any;
      ...
      return { data: null, error: "Passkey authentication failed." };
    } catch (err: any) {
      if (err.name === "NotAllowedError") {
