...
function getDifficultyColor(type: "easy" | "medium" | "hard") {
  switch (type) {
...
export const GeeksForGeeksProfileCard: React.FC<GeeksForGeeksProfileCardProps> = ({
  ...
  const profileUrl = stats?.profile_url || (cleanHandle ? `https://www.geeksforgeeks.org/user/${cleanHandle}/` : `https://www.geeksforgeeks.org/`);
      ...
      <div className="group relative rounded-3xl border border-dashed border-border/70 bg-card/70 backdrop-blur-xl flex flex-col items-center justify-center p-8 text-center min-h-[300px] gap-5 transition-all duration-300 hover:border-green-500/50 hover:shadow-[0_0_30px_rgba(47,141,70,0.15)]">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground max-w-xs">
            Connect your GFG profile to track coding score, problem solving stats, and daily streak.
          </p>
        </div>
        <Button
          size="sm"
    ...
    return (
      <div className="group relative rounded-3xl border border-border/80 bg-card/90 backdrop-blur-xl p-6 sm:p-7 flex flex-col gap-4 min-h-[220px]">
        <div className="flex items-center gap-3.5 border-b border-border/50 pb-4">
          <div className="size-13 sm:size-14 rounded-2xl bg-green-500/10 border border-green-500/20 flex items-center justify-center shrink-0 p-2.5">
            <UnifiedPlatformLogo platform="geeksforgeeks" className="size-7 sm:size-8" />
          </div>
          <div>
      ...
      <div className="space-y-5">
          ...
          <div className="flex items-center gap-3.5 min-w-0">
            <div className="relative shrink-0">
              {stats?.profile_image && !avatarError ? (
                <img
                  ...
                  alt={stats.display_name || cleanHandle}
            ...
            <div className="min-w-0">
              ...
              <a
                ...
                className="text-[11px] text-green-500 hover:underline flex items-center gap-0.5 font-semibold truncate max-w-[200px]"
              >
                @{cleanHandle}
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="p-3 rounded-2xl bg-green-500/10 border border-green-500/20 text-center">
            ...
            <div className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide">Problems</div>
          </div>
          <div className="p-3 rounded-2xl bg-primary/10 border border-primary/20 text-center">
            ...
            <div className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide">GFG Score</div>
          </div>
          <div className="p-3 rounded-2xl bg-orange-500/10 border border-orange-500/20 text-center">
            ...
            <div className={cn("text-xl sm:text-2xl font-black font-mono leading-none", streak > 0 ? "text-orange-500" : "text-foreground")}>{streak}</div>
            <div className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide">Day Streak</div>
          </div>
        </div>
        ...
        <div className="space-y-2.5 p-4 rounded-2xl bg-muted/30 border border-border/50">
          ...
          {[
            { label: "Easy", solved: easySolved, pct: easyPct, color: "emerald", type: "easy" as const },
            { label: "Medium", solved: mediumSolved, pct: mediumPct, color: "amber", type: "medium" as const },
            { label: "Hard", solved: hardSolved, pct: hardPct, color: "red", type: "hard" as const },
          ].map(({ label, solved, pct, color, type }) => (
            <div key={label} className="space-y-1">
              <div className="flex items-center justify-between">
                <span className={cn("text-[11px] font-bold", getDifficultyColor(type))}>{label}</span>
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-muted-foreground">{pct}%</span>
                  <span className={cn("text-xs font-extrabold font-mono", getDifficultyColor(type))}>{solved}</span>
                </div>
              </div>
              <div className="w-full h-1.5 rounded-full bg-muted/60">
                <div
                  className={cn("h-full rounded-full transition-all duration-700", `bg-${color}-500`)}
                  style={{ width: `${pct}%` }}
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {stats?.rank && (
              <div className="p-3 rounded-xl bg-muted/30 border border-border/50 flex flex-col gap-0.5">
                ...
                <span className="text-sm font-extrabold font-mono text-foreground">#{String(stats.rank)}</span>
              </div>
            )}
            {codingScore > 0 && (
              <div className="p-3 rounded-xl bg-muted/30 border border-border/50 flex flex-col gap-0.5">
                <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1">
                  <Award className="size-3" /> Score
                </span>
                <span className="text-sm font-extrabold font-mono text-green-500">{codingScore.toLocaleString()}</span>
              </div>
            )}
          </div>
        )}
