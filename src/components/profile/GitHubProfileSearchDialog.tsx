  ...
  return (
    <>
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side="right" className="w-full sm:max-w-2xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border-l border-slate-200/50 dark:border-slate-800/50 p-0 overflow-hidden flex flex-col z-[70]">
          <SheetHeader className="p-6 pb-2 border-b border-border/50">
            <SheetTitle className="flex items-center gap-2.5 text-lg sm:text-xl font-black text-foreground tracking-tight">
            ...
            <SheetDescription className="text-xs text-muted-foreground">
              Search developers, open-source contributors, and organizations.
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="px-6 py-4 space-y-2 border-b border-border/50">
            ...
            <div className="flex items-center gap-1.5 flex-wrap pt-2">
              <span className="text-[11px] font-bold text-muted-foreground flex items-center gap-1">Popular:</span>
            ...
            <div className="px-6 pb-6 space-y-4">
              ...
              {!loading && !query.trim() && (
                <div className="py-12 text-center text-xs text-muted-foreground space-y-2 border border-dashed border-border/60 rounded-2xl bg-muted/10">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Explore public GitHub profiles, top languages, and repository stats.
                  </p>
                </div>
              )}
              ...
              {!loading && query.trim() && results.length === 0 && !error && (
                <div className="py-12 text-center text-xs text-muted-foreground space-y-2 border border-dashed border-border/60 rounded-2xl bg-muted/10">
                  ...
                  <p className="font-semibold text-foreground">No profiles found for "{query}"</p>
                  <p className="text-[11px] font-mono">Try a different handle or spelling.</p>
                </div>
              )}
              ...
              {results.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-[11px] font-bold text-muted-foreground px-1">
                    <span>Found {totalCount.toLocaleString()} profile{totalCount === 1 ? "" : "s"}</span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {results.map((item) => (
                      <div key={item.login} className="group p-3 rounded-2xl bg-muted/20 border border-border/60 hover:border-blue-500/40 hover:bg-blue-500/5 transition-all flex items-center justify-between gap-3 shadow-sm">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <img src={item.avatarUrl} alt={item.login} className="size-11 rounded-xl object-cover border border-blue-500/20 shrink-0 shadow-sm" />
                          <div className="min-w-0 space-y-0.5">
                            <div className="flex items-center gap-1.5">
                              <span className="font-extrabold text-xs text-foreground truncate group-hover:text-blue-500 transition-colors">@{item.login}</span>
                            ...
                            <p className="text-[10px] text-muted-foreground font-mono truncate">github.com/{item.login}</p>
                          </div>
                        </div>

                        <div className="flex items-center gap-1 shrink-0">
                          <Button size="sm" variant="outline" onClick={() => handleInspectUser(item.login)} className="h-8 text-[11px] font-bold rounded-xl px-2.5 border-blue-500/30 text-blue-600 dark:text-blue-400 hover:bg-blue-500/10 shadow-sm">
                            <span>Inspect</span>
                          </Button>
                          <a href={item.htmlUrl} target="_blank" rel="noopener noreferrer" className="p-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors" title="Open GitHub Profile">
                  ...
                  {results.length < totalCount && (
                    <div className="pt-2 flex flex-col items-center gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={handleLoadMore} disabled={loadingMore} className="rounded-2xl text-xs font-bold gap-2 border-blue-500/30 text-blue-600 dark:text-blue-400 hover:bg-blue-500/10 px-6">
                        {loadingMore ? (<><Loader2 className="size-3.5 animate-spin" /> Loading more...</>) : (<>)Load More Results</>)}
                      </Button>
                    </div>
                  )}
                </div>
      ...
      {/* Inspected User Profile Sheet */}
      <Sheet open={Boolean(inspectingUser)} onOpenChange={(open) => !open && closeInspect()}>
        <SheetContent side="right" className="w-full sm:max-w-3xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border-l border-slate-200/50 dark:border-slate-800/50 p-0 overflow-hidden flex flex-col z-[70]">
          {inspectLoading ? (
            <div className="flex-1 flex items-center justify-center space-y-3 py-16">
              <Loader2 className="size-10 text-blue-500 animate-spin mx-auto" />
              <p className="text-sm font-bold text-foreground">Fetching GitHub statistics...</p>
            </div>
          ) : inspectError ? (
            <div className="flex-1 flex flex-col items-center justify-center py-16 space-y-4">
              ...
              <Button onClick={closeInspect} variant="outline" className="rounded-xl text-xs font-bold">
                Close
              </Button>
            </div>
          ) : inspectedStats ? (
            <div className="flex-1 overflow-y-auto">
              <div className="p-6 border-b border-border/50 pb-5">
                <div className="flex items-center gap-4">
                  <img src={inspectedStats.avatarUrl} alt={inspectedStats.name || inspectedStats.username} className="size-16 sm:size-20 rounded-2xl object-cover border-2 border-blue-500/30 shadow-md shrink-0" />
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-extrabold text-xl text-foreground">{inspectedStats.name || inspectedStats.username}</h3>
                      <span className="text-xs font-mono text-muted-foreground font-semibold">@{inspectedStats.username}</span>
                    </div>
                    {inspectedStats.bio && <p className="text-xs text-muted-foreground max-w-lg leading-relaxed line-clamp-2">{inspectedStats.bio}</p>}
                  </div>
                </div>

                <a href={inspectedStats.htmlUrl || `https://github.com/${inspectedStats.username}`} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex px-4 py-2 rounded-2xl bg-blue-600/10 hover:bg-blue-600/20 text-blue-600 dark:text-blue-400 border border-blue-500/20 font-bold text-xs">
                  <span>View GitHub Profile</span>
                </a>
              </div>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-6 border-b border-border/50">
                <div className="p-3.5 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-center">
                  ...
                  <span className="text-lg font-extrabold text-foreground font-mono block">{inspectedStats.publicRepos}</span>
                ...
                <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-center">
                  ...
                  <span className="text-lg font-extrabold text-foreground font-mono block">{inspectedStats.totalStars ?? 0}</span>
                ...
                <div className="p-3.5 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-center">
                  ...
                  <span className="text-lg font-extrabold text-foreground font-mono block">{inspectedStats.followers}</span>
                ...
                <div className="p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-center">
                  ...
                  <span className="text-lg font-extrabold text-foreground font-mono block">{inspectedStats.following ?? 0}</span>
              ...
              {inspectedStats.topLanguages && inspectedStats.topLanguages.length > 0 && (
                <div className="p-6 border-b border-border/50">
                  <h4 className="font-extrabold text-xs uppercase tracking-wider text-foreground flex items-center gap-1.5 mb-3">
                    <Code2 className="size-4 text-blue-500" /> Top Languages
                  </h4>
                ...
                <div className="p-6 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {inspectedStats.topRepos.slice(0, 4).map((repo) => (
                      <a key={repo.name} href={repo.url} target="_blank" rel="noopener noreferrer" className="p-3.5 rounded-2xl bg-muted/20 border border-border/60 hover:border-blue-500/40 hover:bg-blue-500/5 transition-all space-y-1.5 block">
                        <div className="flex items-center justify-between gap-2">
                          <h5 className="font-bold text-xs text-foreground truncate">{repo.name}</h5>
                          <span className="flex items-center gap-1 text-[11px] font-mono text-amber-500 font-bold"><Star className="size-3 fill-amber-500/30" /> {repo.stars}</span>
                        </div>
                        {repo.description && <p className="text-[11px] text-muted-foreground line-clamp-2 leading-relaxed">{repo.description}</p>}
                      </a>
                    ))}
                  </div>
                </div>
