...
const ALLOWED_LANG_IDS = new Set(['python', 'javascript', 'cpp', 'c', 'java', 'go', 'rust']);
const WORKSPACE_LANGUAGES = SUPPORTED_LANGUAGES.filter(lang => ALLOWED_LANG_IDS.has(lang.id));
...
export interface AISandboxWorkspaceProps {
  ...
  onClose?: (updatedCode?: string, updatedLang?: string) => void;
...
export function AISandboxWorkspace({
  ...
  // Real-time autosave sync with parent component
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* ─── WORKSPACE HEADER ─── */}
      <div className="flex items-center justify-between px-4 py-2.5 bg-card border-b border-border select-none">
        <div className="flex items-center space-x-2">

          <span className="font-bold text-xs uppercase tracking-wider text-foreground">EduSpace Sandbox</span>
        </div>
        ...
        <div className="flex items-center space-x-1">
          ...
          {onClose && (
            <Button
              ...
              variant="ghost"
              size="sm"
              onClick={() => onClose(code, selectedLang.id)}
              className="h-7 w-7 p-0 text-muted-foreground hover:text-rose-400 hover:bg-rose-500/10 transition-all rounded-lg"
              title="Close Workspace & Save Changes"
            >
      ...
      <div className="flex-1 flex flex-col p-3 space-y-3 overflow-y-auto bg-muted/10">
        {/* Editor Container with LeetCode Style Controls & Bottom-Right Run Button */}
        <div className="flex-1 min-h-[350px] relative rounded-xl border border-border bg-card overflow-hidden flex flex-col shadow-sm">
          {/* Top Bar inside Editor */}
          <div className="flex items-center justify-between px-3 py-1.5 bg-muted/40 border-b border-border text-xs select-none">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              <div className="relative">
                <select
                  ...
                  disabled={isRunning}
                  className="h-7 px-2 pr-6 rounded bg-muted/60 border border-input text-[11px] font-medium focus:outline-none focus:ring-1 focus:ring-primary appearance-none cursor-pointer disabled:opacity-50"
                >
              ...
              {/* Copy Code Button — fully visible on mobile & desktop */}
              <Button
                type="button"
                variant="ghost"
                size="sm"
                onClick={handleCopyCode}
                className="h-7 px-2 text-[11px] text-muted-foreground hover:text-foreground flex items-center gap-1 shrink-0"
              ...
              <Button
                ...
                disabled={isRunning}
                className="h-7 px-2 text-[11px] text-muted-foreground hover:text-foreground flex items-center gap-1 shrink-0"
                title="Reset code to original AI state"
              >
          ...
          {/* Editor Body with Bottom Floating Bar */}
          <div className="relative flex-1">
            ...
            {/* Bottom Floating Bar: Left status + Right Run button */}
            <div className="absolute bottom-3 right-3 left-3 z-10 pointer-events-none flex items-center justify-between">
              <div className="pointer-events-auto bg-background/80 backdrop-blur-md px-2.5 py-1 rounded text-[10px] text-muted-foreground font-mono border border-border/40 shadow">
                Auto Saved
              </div>
              ...
              <div className="pointer-events-auto flex items-center gap-2">
                <Button
                  ...
                  disabled={isRunning}
                  size="sm"
                  className="h-8 px-4 font-bold text-xs bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg shadow-xl transition-all active:scale-95 disabled:opacity-60 border border-emerald-400/30"
                >
                  {isRunning ? (
                    <>
            ...
            {isRunning ? (
              ...
              <div className="space-y-2">
                ...
                {!execResult.stdout && !execResult.stderr && !execResult.compileOutput && (
                  <span className="text-muted-foreground italic">Program executed with no terminal output.</span>
                )}
              </div>
            ) : (
