...
export default function StreakPage() {
    ...
    const [activeView, setActiveView] = useState<'present' | 'duel'>(() => {
        const saved = localStorage.getItem('eduspace_streak_view_preference');
        return (saved === 'duel' || saved === 'present') ? saved : 'present';
    });

    const handleViewChange = (view: 'present' | 'duel') => {
        setActiveView(view);
        localStorage.setItem('eduspace_streak_view_preference', view);
    };
                ...
                <ScrollArea className="flex-1">
                    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6 animate-in fade-in duration-500">
                        ...
                        <div className="flex flex-col xl:flex-row gap-6 items-stretch">
                            <div className="flex-1 flex flex-col justify-center space-y-2">
                                <div className="space-y-1">
                                    {activeView === 'present' ? (
                                        ...
                                        <>
                                            <h1 className="text-3xl lg:text-4xl font-black text-slate-900 dark:text-white tracking-tight">
                                                Live Streak <span className="text-primary italic">Arena</span>
                                            </h1>
                                            <p className="text-slate-500 dark:text-slate-400 font-bold text-base max-w-xl">
                                                Challenge classmates, form collaborative clans, and dominate the weekly leaderboard!
                                            </p>
                                        </>
                                    )}
                                </div>
                            ...
                            {activeView === 'present' && (
                                <Card className="xl:w-[450px] border-none shadow-2xl rounded-[2rem] overflow-hidden group shrink-0 transition-all duration-700 animate-in slide-in-from-right duration-500"
                                    style={{
                        ...
                        <div className="flex justify-center pt-2 pb-1">
                            <div className="bg-slate-200/50 dark:bg-slate-900/80 border border-slate-300/30 dark:border-slate-800/60 p-1 rounded-[1.25rem] flex items-center gap-1 shadow-inner relative backdrop-blur-sm">
                                {[
                                    { id: 'present', label: 'Present Streaks', icon: CalendarIcon },
                                    { id: 'duel', label: 'Live Streak Duel', icon: Swords }
                                ].map((tab) => {
                                    const Icon = tab.icon;
                                    ...
                                    return (
                                        <button
                                            key={tab.id}
                                            onClick={() => handleViewChange(tab.id as 'present' | 'duel')}
                                            className={cn(
                                                "relative px-5 py-2.5 text-xs font-black uppercase tracking-wider rounded-xl transition-all duration-300 flex items-center gap-2 z-10",
                                                isActive 
                        ...
                        {/* Content Container — no flip animation, just instant switch */}
                        ...
                        {/* Mobile Touch Hint inside Streak Area (Very small, static, non-glowing caption) */}
                        <div className="w-full text-center mt-4 lg:hidden pointer-events-none select-none">
                            <span className="text-[9px] font-extrabold uppercase tracking-widest text-slate-400 dark:text-slate-500 block">
                                {activeView === 'present' ? 'Tap tabs above to view duels ⚔️' : 'Tap tabs above to view calendar 🔥'}
                            </span>
                        </div>
