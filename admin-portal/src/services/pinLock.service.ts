 ...
 * Ensures consistent, deterministic PIN verification across all admin devices (Laptop, Tablet, Mobile Phone)
...
async function computePbkdf2Hash(
  ...
  const boundSalt = `salt_${salt}:user_${userId || "generic"}:eduspace_admin_sec_v4`;
...
export const pinLockService = {
  ...
  async unlockWithPassword(
    ...
    if (typeof navigator !== "undefined" && !navigator.onLine) {
      return {
        success: false,
        error: "You are offline. Please enter your 4-digit PIN to unlock, or reconnect to the internet to verify your administrator password.",
      };
    }
